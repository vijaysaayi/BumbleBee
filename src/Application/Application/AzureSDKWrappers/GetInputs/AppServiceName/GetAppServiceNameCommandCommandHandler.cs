using Penguin.Application.AzureSDKWrappers.Validation;
        ...
        public async Task<string> Handle(GetAppServiceNameCommand request, CancellationToken cancellationToken)
        {
            ...
            if (appNameAvailability != null)
            {
                ...
                else
                {
                    ...
                    var answer = AnsiConsole.Confirm("Do you want Penguin to add extra characters to make the name your app unique ?");
                    if (!answer)
                    {
